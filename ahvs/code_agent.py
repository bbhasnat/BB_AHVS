...
def _extract_multi_file_blocks(
    ...
    *,
    ...
    "promote first file to main.py" fallback is skipped.
...
class CodeAgent:
    ...
    def _hard_validate(
        ...
        NOTE: Requires ahvs.validator module. If not available, falls back
        to syntax-only checks.
        """
        critical: list[str] = []
        warnings: list[str] = []

        try:
            from ahvs.validator import (
                check_class_quality,
            ...
            # Fallback: syntax-only validation when validator is not available
            for fname, code in files.items():
                ...
                try:
                    ...
                    critical.append(
                        f"[{fname}] Syntax error: {e.msg} (line {e.lineno})"
                    )
